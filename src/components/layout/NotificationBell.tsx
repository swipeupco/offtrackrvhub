...
export function NotificationBell() {
  ...
  async function fetchNotifications() {
    ...
    const { data } = await supabase
      .from('notifications')
      // NOTE: deliberately does not select avatar_url — profiles in this DB store
      // avatars as inline base64 data URIs (~1.7MB each), which would make the
      // bell fetch ~50MB for 30 rows. Falling back to initials in the bell UI;
      // the larger avatar concern is tracked separately for a future cleanup.
